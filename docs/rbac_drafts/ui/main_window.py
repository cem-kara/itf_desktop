...
class MainWindow(QMainWindow):
    ...
    def __init__(self, db=None, session_context=None, authorization_service=None):
        super().__init__()
    ...
    def _is_authorized(self, baslik: str) -> bool:
        permission = PAGE_PERMISSIONS.get(baslik)
        if not permission:
            return True
    ...
    def _on_menu_clicked(self, group, baslik, *args):
        ...
        if not self._is_authorized(baslik):
            QMessageBox.warning(
                self,
                "Yetkisiz Erişim",
                f"'{baslik}' sayfasına erişim izniniz yok."
            )
            return
