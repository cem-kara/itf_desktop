...
class Sidebar(QWidget):
    ...
    def __init__(self, parent=None, session_context=None, authorization_service=None):
        super().__init__(parent)
        ...
        self._page_guard = PageGuard(self._authz, self._session) if self._authz and self._session else None
    ...
    def _can_show_item(self, baslik: str, item: dict) -> bool:
        permission = item.get("permission") or PAGE_PERMISSIONS.get(baslik)
        if not permission:
            return True
        if not self._page_guard:
            return True
        return self._page_guard.can_open(permission)
    ...
    def _load_menu(self):
        ...
        for group_name, items in menu_cfg.items():
            ...
            for item in items:
                ...
                if not self._can_show_item(baslik, item):
                    continue
