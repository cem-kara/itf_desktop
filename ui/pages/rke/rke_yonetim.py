...
class RKETableModel(QAbstractTableModel):
    ...
    def columnCount(self, p=QModelIndex()): return len(_COLS)

    def data(self, index, role=Qt.DisplayRole):
        if not index.isValid(): return None
        row = self._rows[index.row()]
        key = _CK[index.column()]
    ...
    def headerData(self, s, o, role=Qt.DisplayRole):
        if role == Qt.DisplayRole and o == Qt.Horizontal: return _CH[s]
        return None
...
class _GecmisModel(QAbstractTableModel):
    _K = ["Tarih","Sonuç","Açıklama"]
    _H = ["TARİH","SONUÇ","AÇIKLAMA"]

    def __init__(self, parent=None):
        super().__init__(parent); self._rows: List[Dict] = []

    def rowCount(self, p=QModelIndex()):    return len(self._rows)
    def columnCount(self, p=QModelIndex()): return 3
    ...
    def headerData(self, s, o, role=Qt.DisplayRole):
        if role == Qt.DisplayRole and o == Qt.Horizontal: return self._H[s]
        return None
